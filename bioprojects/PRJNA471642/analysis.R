...
# Modify additional metadata
metadata$time_hr <- factor(metadata$time_hr)
...
design <- model.matrix(~0 + time_hr + cell.line, data = metadata)

# DEFINE CONTRAST MATRIX : THIS MUST BE MODIFIED FOR EACH EXPERIMENT
cm <- makeContrasts(
  MEC1_vs_GM12878 = cell.lineMEC1,
  SD6.2hr_vs_SD6.0hr = time_hr2 - time_hr0,
  SD6.6hr_vs_SD6.0hr = time_hr6 - time_hr0,
  SD6.24hr_vs_SD6.0hr = time_hr24 - time_hr0,
  levels = design
)
