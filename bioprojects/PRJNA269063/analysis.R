...
metadata$drug <- factor(regmatches(metadata$group, regexpr("JQ1\\.[0-9]+|DMSO|Ren_[0-9]+|BRD[0-9]_[0-9]+", metadata$group)))
...
qtest <- quantro(logcounts, groupFactor = metadata$drug, B = 1e3)
qaov <- anova(qtest)
...
cm <- makeContrasts(
  JQ1.2 = JQ1.2 - DMSO,
  JQ1.24 = JQ1.24 - DMSO,
  BRD2_1638_vs_DMSO = BRD2_1638 - DMSO,
  BRD3_3235_vs_DMSO = BRD3_3235 - DMSO,
  BRD4_1817_vs_DMSO = BRD4_1817 - DMSO,
  Ren_713_vs_DMSO = Ren_713 - DMSO,
  levels = design
)
