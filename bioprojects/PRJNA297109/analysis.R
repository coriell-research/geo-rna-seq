...
mae <- mae[, !mae$BioSample %in% c("SAMN04115219", "SAMN04115218")]
...
keep_samples <- subset(metadata, group %in% c("HS", "siBRD4_HS"), "BioSample", drop = TRUE)
m2 <- metadata[keep_samples, ]

qtest <- quantro(logcounts[, keep_samples], groupFactor = m2$group, B = 1e3)
qaov <- anova(qtest)
...
cm <- makeContrasts(
  HS_vs_control = HS - control,
  siBRD4_vs_control = siBRD4 - control,
  siBRD4_HS_vs_control = siBRD4_HS - control,
  levels = design
)
