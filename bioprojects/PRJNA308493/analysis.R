...
metadata$drug <- factor(rep(c("EPZ004777", "DMSO"), 2), levels = c("DMSO", "EPZ004777"))
metadata$patient_id <- factor(rep(c("AML_45", "AML_51"), each = 2))
...
design <- model.matrix(~0 + drug + patient_id, data = metadata)

# DEFINE CONTRAST MATRIX : THIS MUST BE MODIFIED FOR EACH EXPERIMENT
cm <- makeContrasts(
  EPZ004777_vs_DMSO = drugEPZ004777 - drugDMSO, 
  levels = design
  )
