...
# Drop outliers from tests -- some have very low avg logCPM values
drop <- rownames(lcpm[rowMeans(lcpm) < -2.5, ])
y <- y[!rownames(y) %in% drop, ]
lcpm <- lcpm[!rownames(lcpm) %in% drop, ]

# Fit models after dropping extreme values
