...
metadata$Drug <- rep(c("None", rep("Chlorprothixene", 3)), 2)
metadata$Treatment <- paste(metadata$Drug, metadata$time_hr, sep = ".")
...
design <- model.matrix(~0 + Treatment + cell.line, data = metadata)

# DEFINE CONTRAST MATRIX : THIS MUST BE MODIFIED FOR EACH EXPERIMENT
cm <- makeContrasts(
  Chlorprothixene.6_vs_None.0 = TreatmentChlorprothixene.6 - TreatmentNone.0,
  Chlorprothixene.12_vs_None.0 = TreatmentChlorprothixene.12 - TreatmentNone.0,
  Chlorprothixene.24_vs_None.0 = TreatmentChlorprothixene.24 - TreatmentNone.0,
  levels = design
)
