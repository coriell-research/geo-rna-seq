...
metadata <- unique(metadata[, .(BioSample, group, cell_type)])
...
# Add additional metadata columns for design
metadata$Treatment <- rep(c("control", "dac"), 6)
metadata$cell_type <- gsub("-", "", metadata$cell_type)
...
qtest <- quantro(logcounts, groupFactor = metadata$cell_type, B = 1e3)
qaov <- anova(qtest)
...
design <- model.matrix(~0 + Treatment + cell_type, data = metadata)

# DEFINE CONTRAST MATRIX : THIS MUST BE MODIFIED FOR EACH EXPERIMENT
cm <- makeContrasts(
  Decitabine_vs_PBS = Treatmentdac - Treatmentcontrol,
  levels = design
)
