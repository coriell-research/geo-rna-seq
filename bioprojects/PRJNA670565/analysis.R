...
metadata <- unique(metadata[, .(BioSample, group, tissue, tumor, treatment)])
...
qtest <- quantro(logcounts, groupFactor = metadata$tissue, B = 1e3)
qaov <- anova(qtest)
...
design <- model.matrix(~0 + treatment + tissue, data = metadata)
colnames(design) <- gsub(pattern = "^treatment", replacement = "", x = colnames(design))

# DEFINE CONTRAST MATRIX : THIS MUST BE MODIFIED FOR EACH EXPERIMENT
cm <- makeContrasts(
  HS.2hr_vs_Control = HS.2hr - Control,
  Estrogen.4hr_vs_Control = Estrogen.4hr - Control,
  levels = design
)
