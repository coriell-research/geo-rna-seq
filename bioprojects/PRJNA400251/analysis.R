...
metadata$Treatment <- rep(c("PBS", "HTT_5", "HTT_10"), 2)
metadata$Source <- rep(c("RAS", "ITD"), each = 3)
...
qtest <- quantro(logcounts, groupFactor = metadata$Source, B = 1e3)
qaov <- anova(qtest)
...
design <- model.matrix(~0 + Treatment + Source, data = metadata)

# DEFINE CONTRAST MATRIX : THIS MUST BE MODIFIED FOR EACH EXPERIMENT
cm <- makeContrasts(
  TreatmentHTT_10_vs_pbs = TreatmentHTT_10 - TreatmentPBS,
  TreatmentHTT_5_vs_pbs = TreatmentHTT_5 - TreatmentPBS,
  levels = design
)
