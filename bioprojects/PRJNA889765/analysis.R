...
# remove doxorubicin sample without a replicate 

metadata<- metadata[!metadata$group %in% "RPE1.doxorubicin.0.34.uM.18.h",]

counts<- counts[,colnames(counts) %in% metadata$BioSample]
...
cm <- makeContrasts(
  RPE1.doxorubicin.0.2uM.18h_vs_DMSO = RPE1.doxorubicin.0.2.uM.18.h - RPE1.DMSO.18.h,
  levels = design
)
