...
metadata$patient <- factor(rep(1:3, each = 2))
...
design <- model.matrix(~0 + group + patient, data = metadata)
...
cm <- makeContrasts(
  Venetoclax_Azacitidine_vs_DMSO_6hr = venetoclax_azacitidine_6hr - untreated_6hr,
  levels = design
)
