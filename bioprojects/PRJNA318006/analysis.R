...
qtest <- quantro(logcounts, groupFactor = metadata$treatment, B = 1e3)
qaov <- anova(qtest)
...
design <- model.matrix(~0 + treatment + mutation, data = metadata)

# DEFINE CONTRAST MATRIX : THIS MUST BE MODIFIED FOR EACH EXPERIMENT
cm <- makeContrasts(
  THP1.Campothecin_vs_DMSO = treatmentCampothecin- treatmentDMSO,
  levels = design
)
