...
## Keep only JQ1 and DMSO samples

Other samples do not have replicates

```{r}
mae <- mae[, mae$drug == "JQ1" | mae$drug == "DMSO"]
mae$group2 <- paste(mae$drug, mae$time_hr, sep = ".")
mae$group2 <- factor(mae$group2)
```
...
metadata <- unique(metadata[, .(bio_sample, group2, cell_line)])
...
```{r}
design <- model.matrix(~0 + group2 + cell_line, data = metadata)
colnames(design) <- gsub(pattern = "^group2", replacement = "", x = colnames(design))
...
fwrite(
  ...
  file = here("results", "PRJNA269063", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA269063", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA269063", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA269063", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA269063", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA269063", "rds-files", "contrast-matrix.rds"))
```
