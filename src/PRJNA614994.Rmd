...
mae <- readRDS(here("data", "PRJNA614994", "MultiAssayExperiment.rds"))
```
...
fwrite(
  ...
  file = here("results", "PRJNA614994", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA614994", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA614994", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA614994", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA614994", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA614994", "rds-files", "contrast-matrix.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA614994", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA614994", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  theme_light() +
  ggsave(here("results", "PRJNA614994", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA614994", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
quickmap(lcpms.sig_genes, 
         ...
         filename = here("results", "PRJNA614994", "figures", "gene-heatmap.png"))
...
#          filename = here("results", "PRJNA614994", "figures", "re-heatmap.png"))
```
...
(gene_vplots[[1]] | re_vplots[[1]]) / (gene_mdplots[[1]] | re_mdplots[[1]]) +
plot_annotation("Gene and RE Expression") +
plot_layout(guides = "collect") +
ggsave(here("results", "PRJNA614994", "figures", "patchwork.png"),
       width = 11, 
       height = 7)
```
...
dotplots[[1]] +
  ggsave(here("results", "PRJNA614994", "figures", "dotplot.png"),
         width = 8,
         height = 5)
```
