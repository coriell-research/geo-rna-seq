...
se <- readRDS(here("data", "PRJNA657067", "summarized_experiment.rds"))
```
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA657067", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(!feature_id %in% cds$gene_name) %>% 
  write_tsv(here("results", "PRJNA657067", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA657067", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA657067", "rds-files", "re-lcpms.rds"))
```
