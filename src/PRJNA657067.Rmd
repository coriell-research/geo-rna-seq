...
mae <- readRDS(here("data", "PRJNA657067", "MultiAssayExperiment.rds"))
```
...
fwrite(
  ...
  file = here("results", "PRJNA657067", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA657067", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA657067", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA657067", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA657067", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA657067", "rds-files", "contrast-matrix.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA657067", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA657067", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  theme_light() +
  ggsave(here("results", "PRJNA657067", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA657067", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
group_colors <- RColorBrewer::brewer.pal(n = nlevels(col_df$group), "Dark2")[1:2]
...
quickmap(lcpms.sig_genes, 
         ...
         filename = here("results", "PRJNA657067", "figures", "gene-heatmap.png"))

quickmap(lcpms.sig_res, 
         annotation_col = col_df,
         annotation_colors = ann_colors,
         main = paste("Significant (FDR <", sig, ") Repetitive Elements"),
         filename = here("results", "PRJNA657067", "figures", "re-heatmap.png"))
```
