...
mae <- readRDS(here("data", "PRJNA396985", "MultiAssayExperiment.rds"))
```
...
```{r}
if (!dir.exists(here("results", "PRJNA396985"))) {
  dir.create(here("results", "PRJNA396985", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA396985", "data-files"))
}
```
...
fwrite(
  ...
  file = here("results", "PRJNA396985", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA396985", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA396985", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA396985", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA396985", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA396985", "rds-files", "contrast-matrix.rds"))
```
