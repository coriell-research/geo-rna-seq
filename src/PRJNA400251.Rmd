...
```{r}
if (!dir.exists(here("results", "PRJNA400251"))) {
  dir.create(here("results", "PRJNA400251", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA400251", "data-files"))
}
```
...
```{r}
mae <- readRDS(here("data", "PRJNA400251", "MultiAssayExperiment.rds"))
```
...
perm_pval <- min(c(perm_pval.source, perm_pval.treatment))
```
...
saveRDS(se, here("results", "PRJNA400251", "rds-files", "se.rds"))
```
