...
quant_files <- list.files(here("data", "PRJNA417314"), 
                          pattern = "quant.sf",
                          recursive = TRUE,
                          full.names = TRUE)
...
```{r}
sra_annotation <- read_csv(here("data", "PRJNA417314", "SraRunTable.txt"))

info <- sra_annotation %>% 
  select(sample_name = Run, treatment = Treatment) %>% 
  mutate(cell_line = "HT29",
         treatment = if_else(str_detect(treatment, "untreated"), "untreated", "5FU_20uM.72hr"))%>% 
...
write_rds(gene_mat, here("results", "PRJNA417314", "rds-files", "gene-mat.rds"))
write_rds(re_mat, here("results", "PRJNA417314", "rds-files", "re-mat.rds"))
```
...
contrast.matrix <- makeContrasts(
  hc29_5fu = HT29.5FU_20uM.72hr - HT29.untreated,
  levels = design
)
...
names(cons) <- cons
titles <- c("HT-29 + 5-FU 20uM 72hr vs. HT-29 Untreated")
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA417314", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(feature_id %in% rownames(re_mat)) %>% 
  write_tsv(here("results", "PRJNA417314", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA417314", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA417314", "rds-files", "re-lcpms.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA417314", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA417314", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  theme_light() +
  ggsave(here("results", "PRJNA417314", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA417314", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
# put colors in list
ann_colors <- list(treatment = trt_colors)
...
quickmap(lcpms.sig_genes, 
         ...
         filename = here("results", "PRJNA417314", "figures", "gene-heatmap.png"))

quickmap(lcpms.sig_res, 
         annotation_col = col_df,
         annotation_colors = ann_colors,
         main = paste("Significant (FDR <", sig, ") Repetitive Elements"),
         filename = here("results", "PRJNA417314", "figures", "re-heatmap.png"))
```
...
(gene_vplots[[1]] | gene_mdplots[[1]]) / (re_vplots[[1]] | re_mdplots[[1]]) +
  plot_annotation("Gene (top row) and RE Expression (bottom row)") +
  ggsave(here("results", "PRJNA417314", "figures", "patchwork.png"),
         width = 18, 
         height = 10)
```
...
dotplots[[1]] +
  ggsave(here("results", "PRJNA417314", "figures", "dotplot.png"),
         width = 8,
         height = 5)
```
