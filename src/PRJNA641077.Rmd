...
mae <- readRDS(here("data", "PRJNA641077", "MultiAssayExperiment.rds"))
```
...
## Collapse reads from both runs

```{r}
counts2 <- sumTechReps(counts, ID = colData(mae)$bio_sample)

# reshape metadata to include only individual samples
metadata <- as.data.table(colData(mae))
metadata <- unique(metadata, by = c("bio_sample", "group"))
...
fwrite(
  ...
  file = here("results", "PRJNA641077", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA641077", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA641077", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA641077", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA641077", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA641077", "rds-files", "contrast-matrix.rds"))
```
