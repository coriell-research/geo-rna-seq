...
pipeline <- function(mae, design, contrast_matrix) {
  ...
  list("design" = design, "metadata" = colData(mae), "raw_counts" = y$counts, "de" = df, "fit" = fit2, "contrast_matrix" = contrast_matrix, "qs" = qs)
}
...
create_se <- function(result) {
  se <- SummarizedExperiment(
    assays = list("counts" = result[["raw_counts"]], "logcounts" = log2(qsmoothData(result[["qs"]]) + 2)),
    colData = result[["metadata"]],
    metadata = list("fit" = result[["fit"]],
                    "de" = result[["de"]]))
}
