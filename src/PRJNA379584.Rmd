...
```{r}
if (!dir.exists(here("results", "PRJNA379584"))) {
  dir.create(here("results", "PRJNA379584", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA379584", "data-files"))
}
```
...
## Create design matrix for each experiment

```{r}
designs <- lapply(experiments, function(experiment) {
  design <- model.matrix(~0 + group, data = colData(experiment))
...
For this analysis assume global differences and apply `QSmooth` pipeline.

```{r message=FALSE}
pipeline <- function(mae, design, contrast_matrix) {
  design <- model.matrix(~0 + group, data = colData(mae))
  colnames(design) <- gsub(pattern = "^group", replacement = "", x = colnames(design))
  y <- DGEList(counts = assay(mae, "counts"), samples = colData(mae))
  ...
  fit2 <- treat(fit, lfc = log2(1.5), trend = TRUE, robust = TRUE)
  res_dts <- get_treat(colnames(contrast_matrix), fit2)
  df <- rbindlist(res_dts, idcol = "contrast")
  
  list("design" = design, "metadata" = colData(mae), "raw_counts" = y$counts, "de" = df, "fit" = fit2, "contrast_matrix" = contrast_matrix)
}

pipeline_res <- parallel::mcmapply(pipeline, experiments, designs, contrast_mats, mc.cores = 12, SIMPLIFY = FALSE)
```
...
```{r}
create_se <- function(result) {
  se <- SummarizedExperiment(assays = list("counts" = result[["raw_counts"]]),
                             colData = result[["metadata"]],
                             metadata = list("design" = result[["design"]], 
                                             "contrasts" = result[["contrast_matrix"]],
                                             "fit" = result[["fit"]],
                                             "de" = result[["de"]]))
}

ses <- lapply(pipeline_res, create_se)
