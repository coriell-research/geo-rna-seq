...
## Calculate distance between all pairs of samples
...
gene_dist <- rdist(t(gene_mat), metric = "euclidean")
re_dist <- rdist(t(re_mat), metric = "euclidean")

# calculate sample-sample correlations
gene_cor <- cor(gene_mat, method = "spearman")
re_cor <- cor(re_mat, method = "spearman")
```
...
# write out
write_tsv(meta_df, here("results", "combined", "data-files", "raw-run-metadata.tsv"))
```

## Read in the manual sample-level annotation

```{r}
sample_metadata <- fread(here("doc", "run-annotation.tsv"), sep = "\t")
