...
metadata <- unique(metadata[, .(BioSample, group, cell.line, treatment, time_hr)])
setDF(metadata, rownames = metadata$BioSample)
```

## DE pipeline

no true replicates. Test for drug effects controlling for cell line.
...
fwrite(
  ...
  file = here("results", "PRJNA511587", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA511587", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA511587", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA511587", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA511587", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA511587", "rds-files", "contrast-matrix.rds"))
```
