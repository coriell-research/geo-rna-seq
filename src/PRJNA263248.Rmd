...
mae <- readRDS(here("data", "PRJNA263248", "MultiAssayExperiment.rds"))
```
...
contrast.matrix <- makeContrasts(
  GNF2_resistant_vs_control = GNF2_resistant - control,
  imatinib_nilotinib_GNF2_resistant_vs_control = imatinib_nilotinib_GNF2_resistant - control,
  imatinib_nilotinib_resistant_vs_control = imatinib_nilotinib_resistant - control,
  imatinib_resistant_vs_control = imatinib_resistant - control,
  nilotinib_resistant_vs_control = nilotinib_resistant - control,
...
```{r}
if (!dir.exists(here("results", "PRJNA263248"))) {
  dir.create(here("results", "PRJNA263248", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA263248", "data-files"))
}
```
...
fwrite(
  ...
  file = here("results", "PRJNA263248", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA263248", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA263248", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA263248", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA263248", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA263248", "rds-files", "contrast-matrix.rds"))
```
