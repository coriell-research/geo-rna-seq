...
mae <- readRDS(here("data", "PRJNA509743", "MultiAssayExperiment.rds"))
```
...
```{r}
if (!dir.exists(here("results", "PRJNA509743"))) {
  dir.create(here("results", "PRJNA509743", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA509743", "data-files"))
}
```
...
fwrite(
  ...
  file = here("results", "PRJNA509743", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA509743", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA509743", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA509743", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA509743", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA509743", "rds-files", "contrast-matrix.rds"))
```
