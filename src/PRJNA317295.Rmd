...
mae <- readRDS(here("data", "PRJNA317295", "MultiAssayExperiment.rds"))
```
...
```{r}
if (!dir.exists(here("results", "PRJNA317295"))) {
  dir.create(here("results", "PRJNA317295", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA317295", "data-files"))
}
```
...
fwrite(
  ...
  file = here("results", "PRJNA317295", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA317295", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA317295", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA317295", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA317295", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA317295", "rds-files", "contrast-matrix.rds"))
```
