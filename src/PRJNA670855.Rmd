...
mae <- readRDS(here("data", "PRJNA670855", "MultiAssayExperiment.rds"))
```
...
fwrite(
  ...
  file = here("results", "PRJNA670855", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA670855", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA670855", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA670855", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA670855", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA670855", "rds-files", "contrast-matrix.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA670855", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA670855", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  ggplot(aes(x = Dim.1, y = Dim.2, color = drug)) +
  geom_point(size = 5) +
  ...
  theme_light() +
  ggsave(here("results", "PRJNA670855", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA670855", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
col_df <- colData(se)[, c("drug"), drop = FALSE] %>% 
  as.data.frame() %>% 
...
# put colors in list
ann_colors <- list(drug = drug_colors)
...
#          filename = here("results", "PRJNA670855", "figures", "gene-heatmap.png"))
# 
# # None significant
...
#          filename = here("results", "PRJNA670855", "figures", "re-heatmap.png"))
```
...
(gene_vplots[[1]] | gene_mdplots[[1]] ) / (re_vplots[[1]] | re_mdplots[[1]]) +
  ...
  ggsave(here("results", "PRJNA670855", "figures", "patchwork.png"),
         width = 18,
         height = 10)
```
