...
se <- readRDS(here("data", "PRJNA670855", "summarized_experiment.rds"))
```
...
## Filter to include only total RNA samples

```{r}
se <- se[, colData(se)$rna_type == "Total RNA"]
```
...
design <- model.matrix(~0 + group, data = colData(se))[, 1:2]
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA670855", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(!feature_id %in% cds$gene_name) %>% 
  write_tsv(here("results", "PRJNA670855", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA670855", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA670855", "rds-files", "re-lcpms.rds"))
```
