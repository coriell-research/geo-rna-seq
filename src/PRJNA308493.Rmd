...
```{r}
colData(mae)$drug <- factor(rep(c("EPZ004777", "DMSO"), 2), levels = c("DMSO", "EPZ004777"))
design <- model.matrix(~patient_id + drug, data = colData(mae))
...
```{r}
res <- setDT(coriell::edger_to_df(glmTreat(fit, coef = 3, lfc = log2(1.5))))
res[, contrast := "EPZ004777_vs_DMSO"]
