...
```{r}
if (!dir.exists(here("results", "PRJNA308493"))) {
  dir.create(here("results", "PRJNA308493", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA308493", "data-files"))
}
```
...
```{r}
mae <- readRDS(here("data", "PRJNA308493", "MultiAssayExperiment.rds"))
```
...
qtest.patient_id <- quantro(counts, groupFactor = metadata$patient_id, B = 1e3)
perm_pval.patient_id <- quantroPvalPerm(qtest.patient_id)
perm_pval <- min(c(perm_pval.drug, perm_pval.patient_id))
```
...
design <- model.matrix(~patient_id + drug, data = metadata)

# Define contrasts to test
contrast.matrix <- makeContrasts(
  EPZ004777_vs_DMSO = drugEPZ004777,
  levels = design
)
...
saveRDS(se, here("results", "PRJNA308493", "rds-files", "se.rds"))
```
