...
metadata$patient_id <- gsub("\\(", "\\.", colData(mae)$patient_id)
...
fwrite(
  ...
  file = here("results", "PRJNA308493", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA308493", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA308493", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA308493", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA308493", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA308493", "rds-files", "contrast-matrix.rds"))
```
