...
mae <- readRDS(here("data", "PRJNA649431", "MultiAssayExperiment.rds"))
```
...
fwrite(
  ...
  file = here("results", "PRJNA649431", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA649431", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA649431", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA649431", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA649431", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA649431", "rds-files", "contrast-matrix.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA649431", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA649431", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  theme_light() +
  ggsave(here("results", "PRJNA649431", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA649431", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
cell_colors <- RColorBrewer::brewer.pal(n = nlevels(col_df$cell_type), "Set1")[1:2]
...
quickmap(lcpms.sig_genes, 
         ...
         filename = here("results", "PRJNA649431", "figures", "gene-heatmap.png"))

quickmap(lcpms.sig_res, 
         annotation_col = col_df,
         annotation_colors = ann_colors,
         main = paste("Significant (FDR <", sig, ") Repetitive Elements"),
         filename = here("results", "PRJNA649431", "figures", "re-heatmap.png"))
```
...
(gene_mdplots[[1]] + scale_y_continuous(limits = c(-10, 10)) |
 ...
 ggsave(here("results", "PRJNA649431", "figures", "gene-patchwork.png"),
        width = 18,
        height = 10)

# RE
(re_vplots[[1]] + scale_y_continuous(limits = c(0, 2)) + scale_x_continuous(limits = c(-2, 2)) |
 re_vplots[[2]] + scale_y_continuous(limits = c(0, 2)) + scale_x_continuous(limits = c(-2, 2)) |
 re_vplots[[3]] + scale_y_continuous(limits = c(0, 2)) + scale_x_continuous(limits = c(-2, 2)) ) /
(re_mdplots[[1]] + scale_y_continuous(limits = c(-2, 2)) |
 re_mdplots[[2]] + scale_y_continuous(limits = c(-2, 2)) |
 re_mdplots[[3]] + scale_y_continuous(limits = c(-2, 2)) ) +
 plot_layout(guides = "collect") +
 plot_annotation("RE Expression") +
 ggsave(here("results", "PRJNA649431", "figures", "re-patchwork.png"),
        width = 18,
        height = 10)
```
...
ggsave(here("results", "PRJNA649431", "figures", "dotplots.png"),
       width = 18,
       height = 7)
```
