...
contrast.matrix <- makeContrasts(
  CAPAN.Calcitriol_vs_CAPAN.DMSO = CAPAN.Calcitriol - CAPAN.DMSO,
  ...
  PDAC.Calcitriol_vs_PDAC.DMSO = PDAC.Calcitriol - PDAC.DMSO,
  PDAC.FFX_vs_PDAC.DMSO = PDAC.FFX - PDAC.DMSO,
...
fwrite(
  ...
  file = here("results", "PRJNA592159", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA592159", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA592159", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA592159", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA592159", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA592159", "rds-files", "contrast-matrix.rds"))
```
