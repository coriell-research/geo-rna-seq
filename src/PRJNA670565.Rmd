...
```{r}
if (!dir.exists(here("results", "PRJNA670565"))) {
  dir.create(here("results", "PRJNA670565", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA670565", "data-files"))
}
```
...
```{r}
mae <- readRDS(here("data", "PRJNA670565", "MultiAssayExperiment.rds"))
```
...
```{r}
qtest.tissue <- quantro(counts, groupFactor = metadata$tissue, B = 1e3)
perm_pval.tissue <- quantroPvalPerm(qtest.tissue)
qtest.treatment <- quantro(counts, groupFactor = metadata$treatment, B = 1e3)
perm_pval.treatment <- quantroPvalPerm(qtest.treatment)
perm_pval <- min(c(perm_pval.tissue, perm_pval.treatment))
```
...
saveRDS(se, here("results", "PRJNA670565", "rds-files", "se.rds"))
```
