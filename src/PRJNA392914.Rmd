...
quant_files <- list.files(here("data", "PRJNA392914"), 
                          pattern = "quant.sf",
                          recursive = TRUE,
                          full.names = TRUE)
...
```{r}
sra_annotation <- read_csv(here("data", "PRJNA392914", "SraRunTable.txt"))

info <- sra_annotation %>% 
  select(sample_name = Run, treatment = Treatment) %>% 
  mutate(cell_line = if_else(str_detect(treatment, "PC9DTP"), "PC9_DTP", "PC9"),
         treatment = case_when(str_detect(treatment, "DMSO") ~ "DMSO",
                               str_detect(treatment, "TSA") ~ "TSA.50nM_5hr",
                               str_detect(treatment, "erlotinib") ~ "erlotinib.1uM_24hr",
                               str_detect(treatment, "carboplatin") ~ "carboplatin.50uM_72hr",
                               TRUE ~ "untreated")) %>% 
...
write_rds(gene_mat, here("results", "PRJNA392914", "rds-files", "gene-mat.rds"))
write_rds(re_mat, here("results", "PRJNA392914", "rds-files", "re-mat.rds"))
```
...
contrast.matrix <- makeContrasts(
  dtp_tsa = PC9_DTP.TSA.50nM_5hr - PC9_DTP.untreated,
  carbo = PC9.carboplatin.50uM_72hr - PC9.DMSO,
  erlo = PC9.erlotinib.1uM_24hr - PC9.DMSO,
  tsa = PC9.TSA.50nM_5hr - PC9.DMSO,
  dtp = PC9_DTP.untreated - PC9.DMSO,
  levels = design
)
...
names(cons) <- cons
titles <- c("PC9_DTP + TSA 50nM 5hrs vs. PC9 + DMSO",
            "PC9 + Carboplatin 50uM 72hr vs. PC9 + DMSO",
            "PC9 + Erlotinib 1uM 24hr vs. PC9 + DMSO",
            "PC9 + TSA 50nM 5hr vs. PC9 + DMSO",
            "PC9_DTP untreated vs. PC9 DMSO")
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA392914", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(feature_id %in% rownames(re_mat)) %>% 
  write_tsv(here("results", "PRJNA392914", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA392914", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA392914", "rds-files", "re-lcpms.rds"))
```
