qs_voom <- function(counts,design=NULL,lib.size=NULL,normalize.method="none",span=0.5,plot=FALSE,save.plot=FALSE,...) 
  ...
  #	Created 22 June 2011.  Last modified 19 April 2019.
{
  ...
  #	Fit linear model to log2-Qsmooth counts (already adjusted for library size)
  y <- log2(counts + 1) 
  ...
  sx <- fit$Amean+mean(log2(lib.size+1))   # Remove -log2(1e6) term
  sy <- sqrt(fit$sigma)
  ...
  fitted.count <- t(t(fitted.cpm)*(lib.size+1))    # Remove 1e-06 factor from original function
  ...
  if(is.null(out$targets))
    ...
    out$targets$lib.size <- lib.size
  if(save.plot) {
    out$voom.xy <- list(x=sx,y=sy,xlab="log2( count size + 1 )",ylab="Sqrt( standard deviation )")
    out$voom.line <- l
  }
