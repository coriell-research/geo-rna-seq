...
message("Reading in gene annotations.")
...
de_results[,  `:=`(id = paste(BioProject, contrast, sep = "."),
                   ...
                   stat = logFC * -log10(FDR),
                   expr = fcoalesce(AveExpr, logCPM))][, 
              `:=`(logCPM = NULL, AveExpr = NULL)]
...
message("Writing out contrasts that require annotations.")
...
# Create SQLite db of DE results and metadata
message("Creating SQLite database of results.")
con <- dbConnect(RSQLite::SQLite(), here("results", "combined", "data-files", "db.sqlite"))
dbWriteTable(con, "results", de_results, overwrite = TRUE)
message("Indexing ID column for SQLite db...")
dbExecute(con, "CREATE INDEX id_idx ON results(id);")
...
if (!all(colnames(lfc_mat) %in% rownames(metadata))) {
  message("Warning: Metadata information missing for some contrasts. Some data may be dropped from final SE object.")
}
...
outlier_contrasts <- c("PDX_CBAB22694_CD45pos.1percent_VTP50469_chow.28d_vs_control",
                       "K562.4sU_labeled_RNA.42_oC_30min_vs_Total_fragmented_RNA.4sU",
                       "K562.4sU_labeled_RNA.42_oC_15min_vs_Total_fragmented_RNA.4sU",
                       "K562.4sU_labeled_RNA.37_oC_vs_Total_fragmented_RNA.4sU",
