...
quant_files <- list.files(here("data", "PRJNA387040"), 
                          pattern = "quant.sf",
                          recursive = TRUE,
                          full.names = TRUE)
...
```{r}
sra_annotation <- read_csv(here("data", "PRJNA387040", "SraRunTable.txt"))

info <- sra_annotation %>% 
  janitor::clean_names() %>% 
  select(run, mutation, treatment) %>% 
  mutate(mutation = if_else(str_detect(mutation, "Wild"), "wt", "ko_cGAS"),
         treatment = if_else(str_detect(treatment, "None"), "control", "etoposide")) %>% 
  unite(group, mutation, treatment, sep = ".") %>% 
  mutate(group = fct_relevel(group, "wt.control")) %>% 
  select(sample_name = run, group)
...
write_rds(gene_mat, here("results", "PRJNA387040", "rds-files", "gene-mat.rds"))
write_rds(re_mat, here("results", "PRJNA387040", "rds-files", "re-mat.rds"))
```
...
contrast.matrix <- makeContrasts(
  wt_eto = wt.etoposide - wt.control,
  ko_eto = ko_cGAS.etoposide - wt.control,
  levels = design
)
...
names(cons) <- cons
titles <- c("Lung Cancer (IMR90 cells) - cGAS KO + Etoposide",
            "Lung Cancer (IMR90 cells) - WT + Etoposide")
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA387040", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(feature_id %in% rownames(re_mat)) %>% 
  write_tsv(here("results", "PRJNA387040", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA387040", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA387040", "rds-files", "re-lcpms.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA387040", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA387040", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  separate(group, into = c("mutation", "treatment"), sep = "\\.") %>% 
  ggplot(aes(x = Dim.1, y = Dim.2, color = treatment, shape = mutation)) +
  geom_point(size = 5) +
  ...
  theme_light() +
  ggsave(here("results", "PRJNA387040", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA387040", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
mut_colors <- c("firebrick", "steelblue")
trt_colors <- c("#1B9E77", "#D95F02")
names(mut_colors) <- unique(col_df$mutation)
names(trt_colors) <- unique(col_df$treatment)

# put colors in list
ann_colors <- list(mutation = mut_colors, treatment = trt_colors)
...
quickmap(lcpms.sig_genes, 
         ...
         filename = here("results", "PRJNA387040", "figures", "gene-heatmap.png"))

quickmap(lcpms.sig_res, 
         annotation_col = col_df,
         annotation_colors = ann_colors,
         main = paste("Significant (FDR <", sig, ") Repetitive Elements"),
         filename = here("results", "PRJNA387040", "figures", "re-heatmap.png"))
```
...
# Gene-level -------------------------------------------------------------------
(gene_vplots[[1]] + scale_y_continuous(limits = c(0, 2)) + scale_x_continuous(limits = c(-15, 15)) | 
 gene_vplots[[2]] + scale_y_continuous(limits = c(0, 2)) + scale_x_continuous(limits = c(-15, 15))) +
plot_annotation(title = "Protein Coding Gene Expression") +
plot_layout(guides = "collect") +
 ggsave(here("results", "PRJNA387040", "figures", "gene-vplots.png"),
        width = 11,
        height = 7)
...
plot_annotation(title = "Protein Coding Gene Expression") +
plot_layout(guides = "collect") +
 ggsave(here("results", "PRJNA387040", "figures", "gene-mdplots.png"),
        width = 11,
        height = 7)

# RE ---------------------------------------------------------------------------
(re_vplots[[1]] + scale_y_continuous(limits = c(0, 2)) + scale_x_continuous(limits = c(-15, 15)) | 
 re_vplots[[2]] + scale_y_continuous(limits = c(0, 2)) + scale_x_continuous(limits = c(-15, 15))) +
plot_annotation(title = "RE Expression") +
plot_layout(guides = "collect") +
 ggsave(here("results", "PRJNA387040", "figures", "re-vplots.png"),
        width = 11,
        height = 7)

(re_mdplots[[1]] + scale_y_continuous(limits = c(-15, 15)) | 
 re_mdplots[[2]] + scale_y_continuous(limits = c(-15, 15))) +
plot_annotation(title = "RE Expression") +
plot_layout(guides = "collect") +
 ggsave(here("results", "PRJNA387040", "figures", "re-mdplots.png"),
        width = 11,
        height = 7)
```
...
(dotplots[[1]] + ggtitle(titles[[1]]) | 
 ...
 ggsave(here("results", "PRJNA387040", "figures", "dotplots.png"),
        width = 11,
        height = 7)
```