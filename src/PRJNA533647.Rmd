...
se <- readRDS(here("data", "PRJNA533647", "summarized_experiment.rds"))
```
...
contrast.matrix <- makeContrasts(
  PCIA_BuCy = PCIA_BuCy - Control,
  PCIA_Bu = PCIA_Bu - Control,
...
names(cons) <- cons
titles <- c("Busulfan + Cyclophosphamide (50 μmol/L, 40 μmol/L) vs Control - 72 hrs",
            "Busulfan 50 μmol/L vs Control - 72hrs")
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA533647", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(!feature_id %in% cds$gene_name) %>% 
  write_tsv(here("results", "PRJNA533647", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA533647", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA533647", "rds-files", "re-lcpms.rds"))
```
