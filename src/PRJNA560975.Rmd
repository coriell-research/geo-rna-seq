...
se <- readRDS(here("data", "PRJNA560975", "summarized_experiment.rds"))
```
...
contrast.matrix <- makeContrasts(
  tedizolid.5uM.4d = tedizolid.5uM - dmso,
  venetoclax.500nM.4d = venetoclax.500nM - dmso,
  venetoclax.500nM_tedizolid.5uM.4d = venetoclax.500nM_tedizolid.5uM - dmso,
...
names(cons) <- cons
titles <- c("Tedizolid 5uM vs DMSO at 4 days", 
            "Venetoclax 500nM vs DMSO at 4 days",
            "Venetoclax 500nM + Tedizolid 5uM vs DMSO at 4 days")
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA560975", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(!feature_id %in% cds$gene_name) %>% 
  write_tsv(here("results", "PRJNA560975", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA560975", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA560975", "rds-files", "re-lcpms.rds"))
```
