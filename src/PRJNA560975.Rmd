...
mae <- readRDS(here("data", "PRJNA560975", "MultiAssayExperiment.rds"))
```
...
fwrite(
  ...
  file = here("results", "PRJNA560975", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA560975", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA560975", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA560975", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA560975", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA560975", "rds-files", "contrast-matrix.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA560975", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA560975", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  theme_light() +
  ggsave(here("results", "PRJNA560975", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA560975", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
col_df <- colData(se)[, c("group"), drop = FALSE] %>% as.data.frame()
...
quickmap(lcpms.sig_genes, 
         ...
         filename = here("results", "PRJNA560975", "figures", "gene-heatmap.png"))

quickmap(lcpms.sig_res, 
         annotation_col = col_df,
         annotation_colors = ann_colors,
         main = paste("Significant (FDR <", sig, ") Repetitive Elements"),
         filename = here("results", "PRJNA560975", "figures", "re-heatmap.png"))
```
...
# gene level
(gene_vplots[[1]] + scale_y_continuous(limits = c(0, 50)) + scale_x_continuous(limits = c(-10, 10)) |
 gene_vplots[[2]] + scale_y_continuous(limits = c(0, 50)) + scale_x_continuous(limits = c(-10, 10)) |
 gene_vplots[[3]] + scale_y_continuous(limits = c(0, 50)) + scale_x_continuous(limits = c(-10, 10)) ) /
...
ggsave(here("results", "PRJNA560975", "figures", "gene-patchwork.png"),
       width = 18,
       height = 10)

# RE
(re_vplots[[1]] + scale_y_continuous(limits = c(0, 40)) + scale_x_continuous(limits = c(-5, 5)) |
 re_vplots[[2]] + scale_y_continuous(limits = c(0, 40)) + scale_x_continuous(limits = c(-5, 5)) |
 re_vplots[[3]] + scale_y_continuous(limits = c(0, 40)) + scale_x_continuous(limits = c(-5, 5)) ) /
(re_mdplots[[1]] + scale_y_continuous(limits = c(-5, 5)) |
 re_mdplots[[2]] + scale_y_continuous(limits = c(-5, 5)) |
 re_mdplots[[3]] + scale_y_continuous(limits = c(-5, 5))) +
plot_annotation("RE Expression") +
plot_layout(guides = "collect") +
ggsave(here("results", "PRJNA560975", "figures", "re-patchwork.png"),
       width = 18,
       height = 10)
```
...
ggsave(here("results", "PRJNA560975", "figures", "dotplots.png"),
       width = 18,
       height = 5)
```
