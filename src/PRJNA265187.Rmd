...
se <- readRDS(here("data", "PRJNA265187", "summarized_experiment.rds"))
```
...
```{r}
keep_cols <- !colnames(se) == "SRR1634333"
se <- se[, keep_cols]
```
...
contrast.matrix <- makeContrasts(
  dac = dac - control,
  dac_siCHD4 = dac_siCHD4 - control,
  siCHD4 = siCHD4 - control,
  siDMNT1 = siDMNT1 - control,
  siDNMT1_siCHD4 = siDNMT1_siCHD4 - control,
  levels = design
)
...
names(cons) <- cons
titles <- c("DAC vs Control",
            "DAC + siCHD4 vs Control",
            "siCHD4 vs Control",
            "siDMNT1 vs Control",
            "siDNMT1 + siCHD4 vs Control")
...
dge <- df %>% 
  ...
  write_tsv(here("results", "PRJNA265187", "data-files", "dge.tsv"))

dre <- df %>% 
  filter(!feature_id %in% cds$gene_name) %>% 
  write_tsv(here("results", "PRJNA265187", "data-files", "dre.tsv"))
```
...
write_rds(lcpms.gene, here("results", "PRJNA265187", "rds-files", "gene-lcpms.rds"))
write_rds(lcpms.re, here("results", "PRJNA265187", "rds-files", "re-lcpms.rds"))
```
