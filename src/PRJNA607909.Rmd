...
```{r}
if (!dir.exists(here("results", "PRJNA607909"))) {
  dir.create(here("results", "PRJNA607909", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA607909", "data-files"))
}
```
...
```{r}
qtest.cell_type <- quantro(counts, groupFactor = metadata$cell_type, B = 1e3)
perm_pval.cell_type <- quantroPvalPerm(qtest.cell_type)
qtest.treatment <- quantro(counts, groupFactor = metadata$Treatment, B = 1e3)
perm_pval.treatment <- quantroPvalPerm(qtest.treatment)
perm_pval <- min(c(perm_pval.cell_type, perm_pval.treatment))
```
...
saveRDS(se, here("results", "PRJNA607909", "rds-files", "se.rds"))
```
