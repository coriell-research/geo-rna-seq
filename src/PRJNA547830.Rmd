...
mae <- readRDS(here("data", "PRJNA547830", "MultiAssayExperiment.rds"))
```
...
```{r}
if (!dir.exists(here("results", "PRJNA547830"))) {
  dir.create(here("results", "PRJNA547830", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA547830", "data-files"))
}
```
...
fwrite(
  ...
  file = here("results", "PRJNA547830", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA547830", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA547830", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA547830", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA547830", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA547830", "rds-files", "contrast-matrix.rds"))
```
