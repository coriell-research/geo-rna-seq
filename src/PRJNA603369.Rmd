...
fwrite(
  ...
  file = here("results", "PRJNA603369", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA603369", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA603369", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA603369", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA603369", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA603369", "rds-files", "contrast-matrix.rds"))
```
...
fviz_eig(pca.gene, addlabels = TRUE) +
  ...
  ggsave(here("results", "PRJNA603369", "figures", "gene-scree.png"),
         width = 8,
         height = 5)

fviz_eig(pca.re, addlabels = TRUE) +
  ggtitle("Repetitive Elements") +
  ggsave(here("results", "PRJNA603369", "figures", "re-scree.png"),
         width = 8,
         height = 5)
...
ind.gene$coord %>% 
  ...
  theme_light() +
  ggsave(here("results", "PRJNA603369", "figures", "gene-pca.png"),
         width = 8,
         height = 5)
# REs
ind.re$coord %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_light() +
  ggsave(here("results", "PRJNA603369", "figures", "re-pca.png"),
         width = 8,
         height = 5)
```
...
quickmap(lcpms.sig_genes, 
         ...
         filename = here("results", "PRJNA603369", "figures", "gene-heatmap.png"))

quickmap(lcpms.sig_res, 
         annotation_col = col_df,
         annotation_colors = ann_colors,
         main = paste("Significant (FDR <", sig, ") Repetitive Elements"),
         filename = here("results", "PRJNA603369", "figures", "re-heatmap.png"))
```
...
# Genes
(gene_vplots[[9]]  + scale_y_continuous(limits = c(0, 20)) + scale_x_continuous(limits = c(-10, 10)) | 
 gene_vplots[[10]] + scale_y_continuous(limits = c(0, 20)) + scale_x_continuous(limits = c(-10, 10)) ) /
(gene_mdplots[[9]]  + scale_y_continuous(limits = c(-10, 10)) |
 gene_mdplots[[10]] + scale_y_continuous(limits = c(-10, 10)) ) +
  plot_layout(guides = "collect") +
  plot_annotation("Protein Coding Gene Expression") +
  ggsave(here("results", "PRJNA603369", "figures", "gene-patchwork.png"),
         width = 18,
         height = 10)

# REs
(re_vplots[[9]]  + scale_y_continuous(limits = c(0, 10)) + scale_x_continuous(limits = c(-4, 4)) | 
 re_vplots[[10]] + scale_y_continuous(limits = c(0, 10)) + scale_x_continuous(limits = c(-4, 4)) ) /
(re_mdplots[[9]]  + scale_y_continuous(limits = c(-4, 4)) |
 re_mdplots[[10]] + scale_y_continuous(limits = c(-4, 4)) ) +
  plot_layout(guides = "collect") +
  plot_annotation("RE Expression") +
  ggsave(here("results", "PRJNA603369", "figures", "re-patchwork.png"),
         width = 18,
         height = 10)
```
...
(dotplots[[9]] | dotplots[[10]] ) +
plot_annotation("RE Family Expression") +
ggsave(here("results", "PRJNA603369", "figures", "dotplots.png"),
     width = 18,
       height = 7)
```
