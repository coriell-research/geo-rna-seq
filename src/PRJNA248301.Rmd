...
mae <- readRDS(here("data", "PRJNA248301", "MultiAssayExperiment.rds"))
```
...
```{r}
if (!dir.exists(here("results", "PRJNA248301"))) {
  dir.create(here("results", "PRJNA248301", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA248301", "data-files"))
}
```
...
fwrite(
  ...
  file = here("results", "PRJNA248301", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA248301", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA248301", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA248301", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA248301", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA248301", "rds-files", "contrast-matrix.rds"))
```
