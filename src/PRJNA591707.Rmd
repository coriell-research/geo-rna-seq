...
mae <- readRDS(here("data", "PRJNA591707", "MultiAssayExperiment.rds"))
```
...
```{r}
if (!dir.exists(here("results", "PRJNA591707"))) {
  dir.create(here("results", "PRJNA591707", "rds-files"), recursive = TRUE)
  dir.create(here("results", "PRJNA591707", "data-files"))
}
```
...
fwrite(
  ...
  file = here("results", "PRJNA591707", "data-files", "dge.tsv"),
  sep = "\t"
  )

fwrite(
  x = df[!feature_id %chin% cds$gene_name],
  file = here("results", "PRJNA591707", "data-files", "dre.tsv"),
  sep = "\t"
)
```
...
saveRDS(lcpms.gene, here("results", "PRJNA591707", "rds-files", "gene-lcpms.rds"))
saveRDS(lcpms.re, here("results", "PRJNA591707", "rds-files", "re-lcpms.rds"))
```

## Save design and contrast matrix

```{r}
saveRDS(design, here("results", "PRJNA591707", "rds-files", "design.rds"))
saveRDS(contrast.matrix, here("results", "PRJNA591707", "rds-files", "contrast-matrix.rds"))
```
